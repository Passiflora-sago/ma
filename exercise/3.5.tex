...
\begin{solution}
    设$x_0$ 是区间 $I$ 的一个内点, 需要证明 $\lim_{x \to x_0} f(x) = f(x_0)$.

    由于$x_0$ 是区间 $I$ 的内点, 故存在 $\delta > 0$, 使得区间 $(x_0-2\delta, x_0 + 2\delta) \subset I$.

    任取$x \in (x_0-\delta, x_0 + \delta)$, 则由凸函数, 有
    $$\frac{f(x_0) - f(x_0-\delta)}{x_0 - (x_0 - \delta)} \les \frac{f(x)-f(x_0)}{x-x_0} \les \frac{f(x_0 + \delta) - f(x_0)}{(x_0 + \delta) - x_0}.$$
    记 $M_1 = \frac{f(x_0) - f(x_0 - \delta)}{\delta}$, $M_2 = \frac{f(x_0 + \delta) - f(x_0)}{\delta}$, 则 $M_1, M_2$ 均为常数, 
    
    因此$$M_1 \les \frac{f(x)-f(x_0)}{x-x_0} \les M_2.$$

    可知对于$M=\max\{|M_1|, |M_2|\}$, 有
    $$-M\left|x-x_0\right| \les\left| f(x) - f(x_0) \right|\les M\left|x-x_0\right|.$$
...
\begin{solution}
    由题设条件以及\hyperref[ex:3.3.13]{习题 3.3.13}知,可得,$f'(x)$单调递增,由一阶导判别法即证.
\end{solution}
...
\begin{proof}
    \begin{enumerate}
        \item 必要性:任取 $x_1, x_2 \in I, x_1 < x_2,$ 任取$x_1 < x < x_2$,对\autoref{thm:3.27} 中第一个不等式,有
    $$
        \frac{f(x) - f(x_1)}{x - x_1} \les \frac{f(x_2) - f(x_1)}{x_2 - x_1}
    $$
    令 $x \to x_1$,有
    $$
       f'(x_1) \les  \frac{f(x_2) - f(x_1)}{x_2 - x_1} .
    $$
    同理,对\autoref{thm:3.27} 中的第二个不等式,有
    $$
        \frac{f(x) - f(x_1)}{x - x_1} \les \frac{f(x_2) - f(x_1)}{x_2 - x_1}
    $$
    并令 $x' \to x_2$,有
    $$
        \frac{f(x_2) - f(x_1)}{x_2 - x_1} \les f'(x_2)
    $$
    所以 $f'(x_1) \les f'(x_2)$.根据 $x_1, x_2$ 的任意性,必须性证明毕.

    \item 充分性:对任意的 $x_1 < x < x_2$,由微分中值定理,存在 $\xi \in (x_1, x), \eta \in (x, x_2)$,使得
    $$
...
\begin{solution}
    \begin{enumerate}
        ...
        \item $y' = \frac{5}{3} x^{2/3}$, $y'' = \frac{10}{9} x^{-1/3}$.
        \begin{itemize}
                  \item 当 $x \in (0, +\infty)$ 时, $y'' > 0$, 函数在 $(0, +\infty)$ 上为凸函数;
                  \item 当 $x \in (-\infty, 0)$ 时, $y'' < 0$, 函数在 $(-\infty, 0)$ 上为凹函数;
              \end{itemize}
              $x = 0$ 为拐点.
        \item $y' = (1+2x+x^2)\mathrm{e}^x$, $y'' = (3 + 4x + x^2)\mathrm{e}^x=(x+1)(x+3)\mathrm{e}^x$.
              \begin{itemize}
        ...
        \item $y' = 1 + \cos x$, $y'' = -\sin x$.
              \begin{itemize}
                  \item 当 $x \in ((2k-1)\pi, 2k\pi), k \in \mathbb{Z}$ 时, $y'' > 0$, 函数在 $((2k+1)\pi, 2(k+1)\pi)$ 上为凸函数;

...
\begin{solution}
    \begin{enumerate}
        \item \begin{align*}
                  y = \frac{x^3 + 2x + 1}{x - 1} & = -(x^3 + 2x + 1)(1 + x + x^2 + \cdots + x^n + o(x^n))  \\
                                                 & = -1 - 3x - 3x^2 - 4x^3 - 4x^4 - \cdots - 4x^n + o(x^n)
              \end{align*}
        \item \begin{align*}
                  y & = \frac{1-\cos 2x}{2} = \frac{1}{2} - \frac{1}{2}\left(1 - \frac{(2x)^2}{2!} + \frac{(2x)^4}{4!} - \frac{(2x)^6}{6!} + \cdots + (-1)^n \frac{(2x)^{2n}}{(2n)!} + o(x^{2n})\right)\\
...
\begin{solution}
    \begin{enumerate}
        \item \begin{align*}
            \tan x& = \tan 0 + \sec^2 0 \cdot x + \frac{2 \tan 0 \sec^2 0}{2!} x^2 + \frac{2(\sec^4 \xi + 2 \tan^2 \xi \sec^2 \xi)}{3!} x^3 \\
            &= x + \frac{\sec^4 \xi + 2 \tan^2 \xi \sec^2 \xi}{3} x^3,\\
            &\text{其中}, x\in\left(-\frac{\pi}{2},-\frac{\pi}{2}\right),\xi=\theta x,0<\theta<1.
        \end{align*}
        \item \begin{align*}
                  \frac{1}{x} & = \frac{1}{-1} + \frac{-1}{(-1)^2}(x + 1) + \cdots + \frac{(-1)^n n!}{(-1)^{n+1} n!}(x + 1)^n + \frac{(-1)^{n+1} (n+1)!}{\xi^{n+2} (n+1)!}(x + 1)^{n+1} \\
                              & = -1 - (x + 1) - (x + 1)^2 - \cdots - (x + 1)^n - \frac{(-1)^{n+1}}{\xi^{n+2}}(x + 1)^{n+1}.\\
                              &\text{其中},x<0,\xi=\theta x-(1-\theta),0<\theta<1.
              \end{align*}
    \end{enumerate}
\end{solution}
...
\begin{solution}
    \begin{enumerate}
        ...
        \item $$\sqrt{1 + x^2} = 1 + \frac{1}{2}x^2 + o(x^2), \quad \sqrt[3]{1 - x^2} = 1 - \frac{1}{3}x^2 + o(x^2)$$
              因此$$\lim_{x \to 0} \frac{\sqrt{1 + x^2} - \sqrt[3]{1 - x^2}}{x^2} = \lim_{x \to 0} \frac{\left(1 + \frac{1}{2}x^2 + o(x^2)\right) - \left(1 - \frac{1}{3}x^2  + o(x^2)\right)}{x^2} = \frac{5}{6}.$$
        \item \begin{align*}
...
\begin{solution}
    \begin{enumerate}
        \item 充分性: 若 $f^{(n+1)}(x) \equiv 0$, 则由 Taylor 公式可知, 考虑在点$x=0$处的Taylor 展开式, 对任意 $x \in \R$, 有
    \begin{align*}
    ...
    \end{align*} 即$f(x)$ 为次数不超过 $n$ 的多项式.
        \item 必要性: 若 $f(x)$ 为次数不超过 $n$ 的多项式, 则可设
...
\begin{solution}
    \begin{align*}
        f(2)& = f(x) + f'(x)(2 - x) + \frac{f''(\eta)}{2}(2 - x)^2, & \eta \in (x, 2).\\
        f(0) &= f(x) + f'(x)(-x) + \frac{f''(\xi)}{2}x^2, & \xi \in (0, x).
    \end{align*}
    两式相减
    $$f(2) - f(0) = 2f'(x)  + \frac{f''(\eta)}{2}(2 - x)^2 - \frac{f''(\xi)}{2}x^2.$$
    因此, 对任意 $x \in [0, 2]$, 有
    \begin{align*}
        |f'(x)| & = \frac{1}{2}\left|f(2) - f(0) + \frac{f''(\xi)}{2}x^2 - \frac{f''(\eta)}{2}(2 - x)^2\right|         \\
                & \les \frac{1}{2}\left(|f(2)|+ |f(0)|+ \frac{|f''(\xi)|}{2}x^2 + \frac{|f''(\eta)|}{2}(2 - x)^2 \right)\\
                & \les \frac{1}{2}\left(1+1 + \frac{1}{2}x^2 + \frac{1}{2}(2 - x)^2 \right)\\
                & = 1 + \frac{1}{4}(x^2 + (2 - x)^2)                                                               \\
                & \les 2
    \end{align*}
\end{solution}
...
\begin{solution}
    ...
    \end{align*}
    但是,当$x\ne 0$时,
    考虑$\{x_k\} ,\{y_k\}$均趋于$x$,且$\forall k\in \N$,$x_k\in \Q,y_k\notin \Q$,则
    \begin{align*}
        \lim_{k\to \infty}f(x_k)&= \lim_{k\to \infty}x_k^{n+1}=x^{n+1},\\
    \lim_{k\to \infty}f(y_k)&= \lim_{k\to \infty}0=0.
    \end{align*}
    因此, $f(x)$ 在$x\ne 0$处不连续,自然$f'(x)$不存在, 故$f''(0)$不存在.
\end{solution}
...
\begin{exercise}[3.6.10]
    ...
    (\text{提示}: 首先, 易用数学归纳法证明, 当 $x \ne 0$ 时, 对 $n=1, 2, \cdots$ 有 $f^{(n)}(x) = \mathrm{e}^{-\frac{1}{x^2}} P_{3n}\left(\frac{1}{x}\right)$, 这里 $P_{3n}(t)$ 是 $t$ 的 $3n$ 次多项式; 其次, 由导数定义及 L'Hôspital 法则, 得出 (\text{记} $y = \frac{1}{x}$)
...
\begin{solution}
    设 $n$ 为自然数, 当 $x \ne 0$ 时, 用数学归纳法可证 $f^{(n)}(x) = \mathrm{e}^{-\frac{1}{x^2}} P_{3n}\left(\frac{1}{x}\right)$, 这里 $P_{3n}(t)$ 是 $t$ 的 $3n$ 次多项式. 
...
\begin{solution}
    不妨设$f^{(n)}(x_0) > 0$,则存在$\delta > 0$,当$|x - x_0| < \delta$时,有$f^{(n)}(x) > 0$.
     由 Taylor 公式, 对任意 $x \in (x_0 - \delta, x_0 + \delta)$, 有
              $$f(x) = f(x_0) + \frac{f^{(n)}(\xi)}{n!}(x - x_0)^n, \quad \xi =\theta x+(1-\theta)x_0,\theta\in(0,1)$$
            \begin{enumerate}
        \item  当 $n$ 为奇数时, 
              \begin{itemize}
